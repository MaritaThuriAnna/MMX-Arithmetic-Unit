...
architecture Behavioral of PMULHW is

  component matr_multiply is
...
begin
  ...
  --inmultesc partile de jos si apoi sus
   m00 : matr_multiply port map(a => lower_A, b => lower_B, p => lower_times_lower);
   m01 :matr_multiply port map(a => upper_A, b => upper_B, p => upper_times_upper);
   -- Extend the result of the multiplication of upper 8 bits of A and B to 32 bits
  upper_times_upper_extended(15 downto 0) <= upper_times_upper(15 downto 0);
   

  --unmultesc a sus b jos si apoi a jos b sus
   m02 : matr_multiply port map(a => upper_A,b => lower_B, p => upper_times_lower);
   m03 :matr_multiply port map(a => lower_A, b => upper_B, p => lower_times_upper);
